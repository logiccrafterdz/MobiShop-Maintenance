  ...
  return (
    <div className="flex h-screen overflow-hidden bg-slate-50 dark:bg-slate-900">
      ...
      <main className="flex-1 flex flex-col p-4 space-y-4 overflow-hidden">
        ...
        {/* Bottom Collapsible Data Table */}
        <section className={`transition-all duration-300 ease-in-out flex flex-col bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden ${isListOpen ? 'flex-1' : 'h-14'}`}>
          <div 
            className="flex items-center justify-between p-4 cursor-pointer select-none"
          ...
          >
            ...
            <button className="p-1 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-full">
              {isListOpen ? <ChevronUp size={20} /> : <ChevronDown size={20} />}
            </button>
          </div>
          {isListOpen && (
